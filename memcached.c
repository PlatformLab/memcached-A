...
#ifdef TIMETRACE
    if (record) {
        timetrace_record("[do_store_item] After do_store_item and logger %d", c->sfd);
    }
#endif
    return stored;
}
...
#ifdef TIMETRACE
    timetrace_set_output_filename("timetrace_scheme4.log");
#endif

#ifdef TIMETRACE_HANDLE
    timetrace_set_output_filename("timetrace_handler_scheme4.log");
#endif
